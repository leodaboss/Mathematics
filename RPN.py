...
def evaluate_in_polish(equation):
    ...
    if equation==None or len(equation)%2==0:return None
    ...
    while len(equation)>1:
        ...
        #once found first symbol, evaluate it's expression
        result=evaluate_simple(equation[counter-2],equation[counter-1],equation[counter])
        
        #replaces operaands and operaator with answer
...
def to_polish(equation,type_number):
    ...
    while i <len(equation)-1:
        if equation[i]=='(':
            ...
            #last encountered was bracket
            end_bracket=True
        elif equation[i] in symbols:
            #find number and makes sure it is correct
            #add number to the list if last wasnt bracket
            if not end_bracket:
                try:
